local M = {
    {
        ...
        config = function()
            ...
            for file, _ in vim.fs.dir(langs_path) do
                ...
                -- 过滤后再合并
                others = __tbl_merge(others, vim.tbl_filter(skip, lang.others or {}))
                others = __tbl_merge(others, vim.tbl_filter(skip, lang.lint or {}))
                others = __tbl_merge(others, vim.tbl_filter(skip, lang.format or {}))
            end

            -- 过滤 servers 列表
            servers = vim.tbl_filter(function(s)
              return not vim.tbl_contains(skip_mason, s)
            end, servers)
